...
<!-- Phil: a much better flow, thanks! -->
...
<!-- Phil: I had no idea you could change the iOS keyboard to Dvorak! (https://www.theverge.com/2022/10/14/23404125/ios-16-dvorak-keyboard-support) -->
...
<!-- Phil: :facepalm: I got lost in trying to work out if you could change the text on the button, which you should be able to do but doesn't work on iOS -->
...
<!-- Phil: I tried to address this above in the line "This is useful in cases where your input is a number, but the input `type` of `number` isn't appropriate, for example with a one time password or a credit card number." -->
...
<!-- Phil: I can't find the original source I had that indicated `readonly` worked with AT, but way back in 2010 Leonie tested a few screenreaders and they worked: https://tink.uk/screen-reader-support-for-disabled-read-only-form-fields/.  This Accessibility guide also says that NVDA will read out a `readonly` form and announce it as such: https://www.accessibility-developer-guide.com/examples/forms/good-example/#announcement-of-controls-attributes -->
...
<!-- Phil: I just tried <audio type="file" capture accept="audio/webm" /> and iOS still gave me video input and the field accepted the video. -->
...
<!-- Phil: There isn't a reference, I actually tested this myself because I couldn't find any documentation on it. Is that ok? -->
...
<!-- I added "names or birth dates" after "personally identifiable information" above to make that clear. -->
...
<!-- Phil: good spot, updated -->

<!-- Eric: I'd love to see an example of two where autocomplete should be explicitly removed. I'm thinking places like OTP input fields, where it just collects a stack of meaningless numbers over time. -->
<!-- Phil: OTP fields should use autocomplete="one-time-password" which browsers don't remember. The only example of this I can think of right now is a text CAPTCHA. Added a paragraph above, let me know what you think. -->
