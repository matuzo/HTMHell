...
Most elements behave the same way regardless of their attributes, the `type` attribute of the `<input>` element can take 1 of 22 different values that gives it not only different behaviour, but a different visual appearance too (many of which are hell to style, of course). The `<input>` element is responsible for everything from text input, through checkboxes and radio buttons, to a button that resets all other elements in the form.

<!-- Eric: I might suggest taking the last sentence in this paragraph and moving it to the beginning so the flow is 1) The input element has a lot of responsibilities it takes care of, 2) this is controlled by its type attribute, and then 3) declaring that type attribute alters the input's behavior and appearance. -->

I want to look beyond the [various types](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input#input_types) that the `<input>` can embody, to the attributes you may not know about. Attributes that tweak the behaviour of the `<input>` to make it more usable, more accessible, and more applicable to various situations. Let's dig in.

## Keyboard control

On a mobile device, focusing a text input will bring up the default virtual keyboard, the familiar qwerty layout. But what if the input you are expecting is numerical or some other format? In this case the [`inputmode` attribute](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input#inputmode) can help to optimise the keyboard for the input you are expecting.

<!-- Eric: Let's delete "the familiar qwerty layout". That may not be everyone's default. -->
...
* "search": the normal keyboard, with a slightly smaller space bar, a period and

<!-- Saptak: I think "search" description is not finished here since the sentence ends with an and? -->

<!-- Eric: Not sure where to place this, but do you think it's worth stating why inputmode can be a better option than type="number" or the like? -->

![An animation showing a browser on iOS tabbing through the different inputmodes, appearing as listed above.](/images/advent2023/1/inputmode.webp)
...
You can also target `readonly` elements in CSS with the pseudoclass `:read-only`. Elements that can be read and written can be targetted with the `:read-write` pseudoclass. 
...
<!-- Eric: Have we tested popular AT support for `readonly`? I know `aria-readonly` is completely unsupported, so this makes me nervous. -->
...
<!-- Saptak: Can you link "`capture` attribute" as well to a reference link. -->
<!-- Saptak: I think it should also be mentioned that this attribute is supported only in the mobile versions of the browsers https://caniuse.com/html-media-capture -->
...
<!-- Saptak: Should we mention here that accept can also be used to capture audio from microphone? -->
...
![An animation showning a file input on a page using the capture attribute set to "user" and the accept attribute set to image types. When touching on the input the camera is opened, facing the user with only photo as an option. An image is captured and it appears as a small thumbnail in the file input.](/images/advent2023/1/capture.webp)
...
Different browsers treat `<input>` elements differently. Firefox and Safari on the desktop will not spellcheck `<input>` elements without the attribute, but Chrome and Safari on iOS will do. To get consistent behaviour across browsers, you should set the `spellcheck` attribute to "true" or "false" depending on the contents of the `<input>`.

<!-- Saptak: "but Chrome and Safari on iOS will do" is there a reference link you can point to regarding this behavior difference? -->
...
One other thing to consider, how spellchecking is implemented is left to the browser. [Chrome and Edge have been known to send the contents of `<input>` elements to a spellchecking service over the network](https://www.otto-js.com/news/article/chrome-and-edge-enhanced-spellcheck-features-expose-pii-even-your-passwords), exposing whatever was in the field. If your `<input>` elements are capturing sensitive data, like personally identifiable information or passwords, then you should set `spellcheck="false"` to avoid sharing it.

<!-- Saptak: I really appreciate this point of letting the readers know about the privacy concerns! -->

<!-- Eric: Agreed! I might also suggest disabling spellcheck explicitly for situations where folks need to input conent like their name. It feels weird to have an OS treat your name as invalid if it's not part of the default OS dictionary. -->
...
Correctly using the `autocomplete` attribute is even a [sufficient technique](https://www.w3.org/WAI/WCAG21/Techniques/html/H98) to fulfill [the WCAG level AA rule "identify input purpose"](https://www.w3.org/WAI/WCAG21/Understanding/identify-input-purpose). Providing fine-grained hints to the browser for the type of content the input fields are expecting means that they can be filled in saving time and typing for all users.

<!-- Saptak: Should it link to WCAG 2.2 now that it's a recommendation though I don't think there is any difference in this criterion -->

<!-- I'd love to see an example of two where autocomplete should be explicitly removed. I'm thinking places like OTP input fields, where it just collects a stack of meaningless numbers over time. -->
