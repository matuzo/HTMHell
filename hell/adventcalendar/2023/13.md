...
<p class="highlight">Please keep in mind that `role="status"` and `role="alert"` are the only two live region roles that have good browser/screen reader support.</p>
...
For example, an element with `role="alert"` has an implicit `aria-live="assertive"` property and should (in theory) inform the screen reader that the notification in that area is really an alert (although it may not work will all screen reader/browser combinations). While the `aria-live="assertive"` attribute by itself will inform the screen reader that it should interrupt whatever it is reading to announce that notification.
