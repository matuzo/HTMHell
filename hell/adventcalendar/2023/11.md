...
<!-- Manuel: link to wikipedia or similar please -->
...
<!-- Manuel: Why exactly is JS the "the most powerful language"? I feel like this needs explanation because _powerful_ can mean a lot and mean different things depening on the context-->
...
<!-- 
  Eric:
  >and your end users will have a better experience.
  I might suggest removing this bit, in that some parts of HTML and CSS are inacessible by default and will not make a better experience 😬 
-->
...
<!-- Eric: One thing I've been thinking a lot about is I don't think the appeal to backwards compatability is as compelling as we think it is. Folks are more concenred in the immediate about it working while they build it, so I wonder if we could rephrase this to communicate the point you're driving at in a more compelling way? -->
...
<!-- Eric: Do you mean a switch? Asking because if we're building a toggle I'd suggest buttons instead: https://tink.uk/perceived-affordances-and-the-functionality-mismatch/ -->
...
<!-- 
  ...
  We could probably cut this and the paragraph would still read the same. Simple to folks like us, but there might be a reader who is learning this for the first time.
-->

Of course, designers may not like the way this looks and we want to create a great looking custom toggle. So let's add a bunch of CSS:
...
<!-- Manuel: we should show the result here. Please tweak the styles if necessary. -->
...
.toggle2:focus-visible {
  outline: 2px solid dodgerblue;
  outline-offset: 2px;
}
</style>

<div data-demo="custom checkbox">

<p>
  <label>
    <input type="checkbox" class="toggle toggle1">
    My awesome feature
  </label>
</p>
...
<!-- Eric: Could we link to the spec definiton or the like of replaced content? -->
...
<!-- 
  Eric:
  >Appearance is a way of telling the browser to stop doing that, to say to the browser:
  Suggested rephrase:
  `appearance` is a way of telling the browser to stop doing that. It tells the browser:
-->  

Clicking this still checks and unchecks the checkbox, but because we replaced the element, we need to do the work of making that visible. That's where the `:checked` pseudo-class comes in:
...
<!-- 
  Eric:
  >Of course if you're reading this you know that's not a good idea. 
  Suggested rephrase:
  If you're reading this, know that it's not a good idea.
  Let's avoid assumptive phrases!
-->
...
<!-- Manuel: what about the `outline: 2px solid dodgerblue` from the :focus-visible rule. Isn't that enough for forced colors mode? -->
...
<p>
  <label>
    <input type="checkbox" class="toggle toggle2">
    My awesome feature
  </label>
</p>
...
<!-- Manuel: it's worth mentioning that there's also the <button aria-pressed="true"> approach, and how it 
differs from yours.
see: https://adrianroselli.com/2019/08/under-engineered-toggles-too.html -->
...
<!-- Manuel: please consider linking https://www.webaxe.org/datalist-over-aria-combobox/ and/or https://adrianroselli.com/2023/06/under-engineered-comboboxen.html for more details -->

<!-- Eric: I believe VoiceOver stilld does not support datalist 😬 -->
...
<!-- Eric: Last I checked the input-triggered color pickers weren't fully accessible. This may have changed, however. It might be worth checking and leaving a note. -->
...
<!-- Eric: I believe the details/summary has a lot of assistive technology weirdness, especially with custom summary markers. It might be worth evaluating this with VoiceOver and NVDA before suggesting it.
Manuel: I've got you covered ;) https://www.matuzo.at/blog/2023/details-summary -->
...
<!-- Manuel: so does that mean that we don't ever need a JS version again or are there downsides to this approach?
...
<!-- Eric: Can we link to a spec definiton of what top-layer is? -->
...
<!-- Eric: Bold choice with the values there 😀 -->
...
<!-- Eric: Could we link to the MDN or spec for what formData is? -->
...
<!-- Manuel: "Because the dialog is essentially a div" <- regarding sematics as well? -->
...
<!-- Manuel: consider adding a demo here, as well -->
...
<!-- Eric: Could we link to resources for the examples you list off for the curious as a jumping off point? -->
...
So let me re-iterate the main point of this article: Just because you _know_ something needs JavaScript, doesn't mean it still does. You can make better websites if you test those assumptions every now and then.

<!-- Eric: If I might make one high-level suggestion here, it's to also perform at least some cursory testing with assistive technology to make sure the non-JS solution delivers on its promise. -->
