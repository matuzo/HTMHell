...
<!-- 
  Eric:  
  Hey Chris! I think this is a fine article, and thank you for writing it. The one piece of feedback I have is that I'd love to see new JS terms linked to int he MDN or spec. This might be a reader's first time reading about these concepts, so I think it'd be helpful to do the hard work to make it easy and point the reader to a reputable source.
-->
...
<!-- Manuel: Nice! I like that comparison. -->
...
<!-- Manuel: Maybe I'm nitpicking, but HTMLElement is a DOM interface not just a "simple" object. When I learned about it, I found this sentence in the spec useful "The basic interface, from which all the HTML elements' interfaces inherit" https://html.spec.whatwg.org/multipage/dom.html#elements-in-the-dom 
Adding that detail can be helpful because later you say "In it, we'll run the super() method to make sure we have access to the parent class (in this case, the HTMLElement object) properties."-->

<!-- Eric: Could we link to the MDN or spec page for `customElements.define()`? One thing I like to do is proactively include a link as a reference. It does double-duty: one to provide more info for the curious and two to establish that it is an, er, established and actual thing 😅 -->
...
<!-- Eric: Can we also link to what `constructor()`, `HTMLElement`, and `super()` are? Same sentiment as my previous comment. -->
...
<!-- Eric: Can we also link a decent read on what `this` is, as well as what component state is? The latter is jargon a reader might not be familiar with. -->
...
<!-- Eric: Where can I go to learn more about what lifecycle functions are? -->
...
<!-- Manuel: Not if you use an arrow function, or are there good reasons not to use an arrow function?  -->
...
<!-- Eric: I might suggest removing the "moved" part in this comment, in that while it is technically part of what is supported, I think it makes describing the example less direct. I now have questions as the reader about what specifically constitutes "moving" int the DOM. -->
...
<!-- 
  Eric:  
  >each one is self-contained and encapsulated
  Might be worth quickly reiterating how it is self-contained and encapsulated.
-->
...
<!-- 
  ...
  I'm now wondering what those reasons are. Is this something I need to worry about? Can I accidentally break things somehow or make them worse?
-->
...
<!-- Manuel:
Hidden in which regard? -->

<!-- Eric: Can we link to a high-level explainer of what the Shadow DOM is? -->
...
<!-- Manuel:
any thoughts on open vs. closed shadow dom. I've often heard that closed shadow dom is a bad practice -->
...
<!-- Eric: I like this summary! Concise and to the point. -->
