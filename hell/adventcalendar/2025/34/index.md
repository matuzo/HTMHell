...
If you've been riding along for some time on the web, you know the `<strong>` and `<em>` tags and you probably use them frequently. They are quite nice:
...
Especially when working with content management systems, like we do at our agency, they are used a lot inside user content. I have always learned that using `<strong>` and `<em>` is the "better option" when it comes to marking pieces of text as important. The semantic way; the _correct_ way. This is why I was very surprised when some of our websites were recently audited with the official WCAG [Evaluation Methodology](https://www.w3.org/WAI/test-evaluate/conformance/wcag-em/), we kept receiving issues that we were over-using these tags, and that we should switch to `<b>` and `<i>`.
...
So, the truth is, I kind of forgot about `<b>` and `<i>`. And not only me: even to this day, there is no way to insert `<b>` and `<i>` tags when using the default editors of popular content management systems like WordPress and Drupal. As a non-developer, without writing code, you just can't do it. You can only insert `<strong>` and `<em>` tags in common text editors. Ironically, the design of the buttons that add these elements is always with the bold letter B and the italic letter I, as can be seen in the screenshots below.

<figure>

![Screenshot of the WordPress Classic Editor](https://ps.w.org/classic-editor/assets/banner-1544x500.png?rev=1998671)

<figcaption>The WordPress Classic Editor's default text editing interface has several formatting options. Among many options, the 'bold' and 'italic' options are visible, which will output 'strong' and 'em' HTML tags. There are no separate options for 'b' or 'i' tags.</figcaption>
</figure>
...
<figcaption>CKEditor5, which is used in the Drupal Core, has several formatting options in the editing interface. Among many options, the 'bold' and 'italic' options are visible, which will output 'strong' and 'em' HTML tags. There are no separate options for 'b' or 'i' tags.</figcaption>
</figure>
...
The key difference between these tags lies in how a person perceives the text: <b>visually or non-visually</b>. The problem is that right now, in many cases, `<strong>` and `<em>` tags are used as one single solution to two different approaches. You should only use `<strong>` and `<em>` tags if you really want to signal importance or emphasise a text, both when visually reading it and when the text is read to you. By assistive technology, for example.

Semantics and accessibility overlap but are not identical: semantics describe meaning, while accessibility ensures that meaning is conveyed appropriately to all users

If you feel the need to use an HTML element for extra emphasis, ask yourself this:
...
If the answer is no: congratulations! You found a use case for `<b>` and `<i>` tags. Or you could use any other HTML element and apply custom `font-weight` styling. This is exactly what the WCAG auditor meant with the feedback we got. You intend to make the text only _visually_ emphasized, not audibly.

If the answer is still yes: hang on for a second and read on to learn why you still might want to choose `<b>` or `<i>`.
...
```html
We hereby would like to invite <strong>all</strong> of our colleagues and
clients to a <strong>special event</strong> that will only take place
...
restaurant serving <strong>100% local food</strong>.
```
...
We all tried to do the right thing. Unfortunately, we are left with the end result where `<strong>` and `<em>` tags are so over-used that they no longer "work" as intended.
...
This definitely does not mean `<strong>` and `<em>` should never be used. Especially inside written content like articles and blogs, there may be cases where you certainly mean to emphasize. You can definitely still do that. Even if it is mostly ignored by screen readers (you can still [enable it](https://www.tpgi.com/screen-readers-support-for-text-level-html-semantics/#:~:text=All%20are%20off%20by%20default%2C%20but%20can%20be%20enabled%20though%20user%20settings.) if you want), it may still be of value to other text processing systems that recognize HTML. Just don't over-use it.
